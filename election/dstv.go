...
func init() {
	RegisterTally(&TallyDSTV{})
}

type TallyDSTV struct {
	Droop  int
	e      *Election
	Score  VoteM
	Ignore Ints
...
func (t *TallyDSTV) Tally(e *Election) []int {
	t.e = e
	return t.DSTV()
}

func (*TallyDSTV) Key() string {
	return "dstv"
}

// Returns DSTV result
func (t *TallyDSTV) DSTV() []int {
	e := t.e
...
func (t *TallyDSTV) distribute(i int) {
	ratio := float64(1)
...
func (t *TallyDSTV) after(k int) VoteM {
...
func (t *TallyDSTV) first(v *Vote) int {
	for i := 0; i < len(v.C); i++ {
